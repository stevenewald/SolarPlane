...
with navio.pwm.PWM(1) as throttle:
    with navio.pwm.PWM(2) as elevator:
        with navio.pwm.PWM(3) as rudder:
            with navio.pwm.PWM(4) as spoiler:
                ...
                rcin = navio.rcinput.RCInput() #RC Inputs
                ...
                while (Takeoff): 
                    ...
                    ################## RCINPUT ###################################
                    if not(override):
                        adjustment = (abs(x-90)**1.5)/853
                        if x < 0:
                            adjustment = adjustment*(-1)
                        elevatorAngle = max(1, min(1+adjustment, 2))
                    ...
                    if ((tick % 10) == 0): #only every 10 ticks
                        ...
                        print("adjustment: " + str(elevatorAngle))
                        print("Heading: " + str(z))
